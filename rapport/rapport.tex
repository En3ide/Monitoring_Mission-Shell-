...
Toutes les données récoltées par l'application sont écrites et mises à jour dans un fichier de sortie \texttt{logs.txt} appelé le \textbf{fichier de logs} (logfile).
\\
Certains aspects graphiques et techniques comme les couleurs ou l'intervalle d'écriture dans le logfile sont personnalisables à l'aide d'un fichier d'entré \texttt{config.txt} appelé le \textbf{fichier de configuration} (configfile).

% 2. Algorithme, fichier et fonctions
\section{Algorithme, fichiers et fonctions}
Le projet est séparé en 4 fichiers :
\begin{itemize}
    \item \texttt{main.sh} : lit le fichier de configuration et lance le programme principale.
...
\begin{itemize}
    \item \texttt{main.sh} :
        \begin{itemize}[label=\textbullet]
            \item \texttt{config\_file} : lit le fichier de configuration et met à jour les valeurs par défaut de l'interface.
        \end{itemize}
    \vspace{0.3em}
    ...
    \item \texttt{recup\_info.sh} :
        \begin{itemize}[label=\textbullet]
            ...
            \item \texttt{recup\_cpu} : récupère les informations sur la carte graphique de la machine.
            \item \texttt{recup\_gpu} : récupère les informations sur le processeur de la machine.
            \item \texttt{recup\_disk} : récupère les informations sur les disques de la machine.
...
Des possibles améliorations seraient par exemple d'utiliser des processus asynchromes (multi-processing), ou adapter les nombres de colonnes des processus à la taille du terminal.
