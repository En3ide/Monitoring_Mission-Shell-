...
info_reduite() { # Jamel Bailleul & Tim Lamour
    ...
    if [[ -n "$inter_name" && $(tput lines) > $(( $1 + ( 3 * position ) + 9 )) ]]; then
        ...
        for (( i=1 ; i <= $nb_inter ; i++)); do
            ...
            echo -en "${!bg_color}${!font_color}Lost Packets ${unicode_down_row}: ${reset}"
            ...
            echo -en "${!bg_color}${!font_color}Lost Packets ${unicode_up_row}: ${reset}"
            percent=$(calculate_percent $(recup_network "uploadErr" $name) $(recup_network "uploadPackets" $name))
